            ...
            <div className="card bg-base-100 shadow-xl" key={item._id}>
                <figure>
                        ...
                        <span className={`absolute top-0 right-0 font-bold text-white rounded 
                            ...
                            <a className="m-1">
                                ({item.ticket_status === "Free" ? "Free" : "Paid"})
                            </a>
                        </span>
                ...
                <div className="card-body">
                    ...
                    {user && user.role === 'admin' && (
                        <div className="flex justify-end gap-x-3">
                            <FaTrash  onClick={ async () => { await customAPI.delete(`/program/${item._id}`); toast.success('Success delete program!'); revalidate(); }} className="text-red-500 cursor-pointer" />
                            <Link to={`/program/${item._id}/update`}>
                    ...
                    <p className="flex items-center gap-2 text-xs font-bold"><FaCalendarAlt />{formatDate(item.deadline_registration)}</p>
                    ...
                    <Link to={`/program/${item._id}`} className="btn btn-neutral bg-blue-600 text-white mt-2">See more</Link>
                    </div>
                </div>
            </div>
        </>
